...
namespace simialbi\yii2\formbuilder\widgets;

use simialbi\yii2\formbuilder\JsonEditorAsset;
    ...
    public function registerClientScript()
    {
        ...
        if (!empty($this->collapseAll) || !empty($this->expandAll)) {
            if (isset($this->clientOptions['onModeChange'])) {
                $userFunction = " var userFunction = {$this->clientOptions['onModeChange']}; userFunction.call(this, newMode, oldMode);";
            } else {
                $userFunction = '';
            }
            ...
            foreach (['collapseAll', 'expandAll'] as $property) {
                if (!empty($this->$property)) {
                    $jsOnModeChange .= 'if (' . Json::htmlEncode($this->$property) . '.indexOf(newMode) !== -1) ' . "{{$editorName}.$property();}";
                }
            }
        ...
        $jsCode = "$editorName = new JSONEditor(document.getElementById('{$this->containerOptions['id']}'), " . Json::htmlEncode($this->clientOptions) . ", $encodedValue);\n" . "jQuery('#$hiddenInputId').parents('form').submit(function() {{$jsUpdateHiddenField}});";
